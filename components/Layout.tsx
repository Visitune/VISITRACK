...
import {
  ...
  AlertCircle, CheckCircle2, Send, Sun, Moon, Truck
} from 'lucide-react';
  ...
  return (
    <div className="relative">
      <button
        ...
        className="relative p-2 rounded-lg hover:bg-[var(--bg-hover)] transition-all group"
      >
        <Bell className="w-5 h-5 text-[var(--text-secondary)] group-hover:text-[var(--accent)]" />
        {unreadCount > 0 && (
          <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-[var(--accent)] rounded-full border border-[var(--bg-card)]"></span>
        )}
      </button>
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 mt-3 w-80 bg-[var(--bg-card)] border border-[var(--border-subtle)] shadow-xl z-40 overflow-hidden rounded-xl animate-fade-in">
                ...
                notifications.map((n) => {
                  ...
                  return (
                    <div key={n.id} className={`p-4 flex gap-3 hover:bg-[var(--bg-main)] transition-all border-b border-[var(--border-subtle)] last:border-0 relative group ${!n.isRead ? 'bg-[var(--accent)]/5' : ''}`}>
                      <div className={`shrink-0 w-8 h-8 ${styles.bg} flex items-center justify-center`}>
    ...
    <div className={`flex h-screen bg-[var(--bg-main)] text-[var(--text-primary)] font-sans transition-colors duration-300`}>
      ...
      <aside className="w-64 bg-[var(--bg-sidebar)] border-r border-[var(--border-subtle)] flex flex-col z-20 shrink-0 transition-colors duration-300">
        <div className="p-6 flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-[var(--accent)] flex items-center justify-center shadow-lg shadow-orange-500/15 overflow-hidden">
          ...
          <span className="text-sm font-bold tracking-tight text-[var(--text-primary)]">VISITrack</span>
        </div>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 mx-3 px-3 py-2.5 text-[12px] font-medium rounded-lg transition-all group w-[calc(100%-24px)] ${isActive
                  ? 'nav-item-active text-[var(--accent)]'
                  : 'text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)]'
                  }`}
              >
                <item.icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-[var(--accent)]' : 'text-[var(--text-muted)] group-hover:text-[var(--text-primary)]'}`} />
                {item.label}
              </button>
            );
          })}
        ...
        <div className="p-4 mt-auto border-t border-[var(--border-subtle)] space-y-1">
          <button onClick={exportWorkspace} className="w-full flex items-center gap-3 px-4 py-2 text-[10px] font-bold uppercase tracking-widest hover:bg-[var(--bg-main)] transition-all text-[var(--text-secondary)]">
            <Download className="w-3.5 h-3.5" /> Exporter Pack
          </button>
          <button onClick={() => fileInputRef.current?.click()} className="w-full flex items-center gap-3 px-4 py-2 text-[10px] font-bold uppercase tracking-widest hover:bg-[var(--bg-main)] transition-all text-[var(--text-secondary)]">
            <Upload className="w-3.5 h-3.5" /> Importer Pack
          </button>
          <button onClick={resetWorkspace} className="w-full flex items-center gap-3 px-4 py-2 text-[10px] font-bold uppercase tracking-widest hover:text-orange-500 hover:bg-orange-500/5 transition-all text-[var(--text-muted)]">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-16 px-8 flex justify-between items-center bg-[var(--bg-card)] border-b border-[var(--border-subtle)] z-10">
          ...
          <div className="flex items-center gap-4">
            <button
              onClick={toggleTheme}
              className="p-2 rounded-lg hover:bg-[var(--bg-hover)] transition-all text-[var(--text-secondary)]"
            ...
            <NotificationCenter />
            <div className="w-px h-6 bg-[var(--border-subtle)]" />
            <div className="w-8 h-8 bg-[var(--accent)] text-white flex items-center justify-center font-bold text-xs rounded-lg shadow-md shadow-orange-500/15">
