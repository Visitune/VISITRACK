            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col h-full">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <label className={`px-4 py-2 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest cursor-pointer hover:bg-indigo-700 transition-all flex items-center gap-2 ${isAnalyzing ? 'opacity-50 cursor-not-allowed' : ''}`}>
                            <Upload className="w-4 h-4" />
                            {isAnalyzing ? '...' : 'Upload'}
                            {!isAnalyzing && <input type="file" className="hidden" onChange={handleCertificateUpload} />}
                        </label>
                    </div>

                    <div className="space-y-4 flex-1">
                        {supplier.gfsiCertificates?.map(cert => (
                            <div key={cert.id} className="p-5 bg-slate-50 rounded-2xl border border-slate-100 group relative">
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col h-full">
                    ...
                    <div className="space-y-3 flex-1">
                        {isAddingMaterial && (
                            <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 space-y-3 mb-4 animate-in slide-in-from-top-2">
                                <input
                                    className="w-full bg-white border border-indigo-200 rounded-xl p-2 text-xs font-bold outline-none"
                                ...
                                <div className="flex gap-2">
                                    <select
                                        className="flex-1 bg-white border border-indigo-200 rounded-xl p-2 text-[10px] font-black outline-none"
                                    ...
                                    <button onClick={handleAddMaterial} className="p-2 bg-indigo-600 text-white rounded-xl"><Plus className="w-4 h-4" /></button>
                                    <button onClick={() => setIsAddingMaterial(false)} className="p-2 bg-white text-slate-400 rounded-xl"><XCircle className="w-4 h-4" /></button>
                                </div>
                            </div>
                        )}
            ...
            <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm relative overflow-hidden">
                ...
                <div className="flex justify-between items-center mb-8 relative z-10">
                    ...
                    <button
                        ...
                        className="px-6 py-3 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-3 shadow-xl hover:bg-black transition-all"
                    >
